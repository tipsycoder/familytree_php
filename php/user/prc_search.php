...
function createSearchTable($searchedArray, &$errMsgs, $sort, $sortBy, $keyword, $opt) {
    ...
    if(count($searchedArray) > 0) {
        ...
        $htmlStr .= "<tr class='tableHeader'><td>EMAIL</td><td id='fNameSort' onclick='searchNameSort(\"$sort\" , \"FNAME\", \"../user/prc_search.php?uSearch=$keyword&uOptions=$opt\")'>FIRST NAME". getSpaces(2) ."<i class='fa $fNameCaret'></i></td><td id='lNameSort' onclick='searchNameSort(\"$sort\" , \"LNAME\", \"../user/prc_search.php?uSearch=$keyword&uOptions=$opt\")'>LAST NAME". getSpaces(2) ."<i class='fa $lNameCaret'></i></td></tr>";
        for($i = 0; $i < count($searchedArray); $i++) {
            $email = $searchedArray[$i]['email'];
            $fName = $searchedArray[$i]['firstName'];
            $lName = $searchedArray[$i]['lastName'];

            if(strtolower($_SESSION['user']['email']) === strtolower($email)) {
                continue;
            }

            $htmlStr .= "<tr ><td style='padding-left: 5px'>$email</td><td>$fName</td><td>$lName</td><td><a href='tree.php?email=$email'><button>See Tree</button></a></td><td><a href='childForm.php?em=$email&fn=$fName&ln=$lName'><button type='button'>Add Child</button></a></td></tr>";
            $recordCnt++;
        }
        $htmlStr .= "</table>";
    }
